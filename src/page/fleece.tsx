...
export class FleeceComponent extends React.Component {
  constructor(props: any) {
    ...
    this.state = {
      ...
      repoUrl: `${Config.userName}@github`,//local
      private: false,
      api: {
  ...
  _readStorage(): void {
    console.log('1');/* eslint no-console: 0*/
    ...
    if (moment.utc().valueOf() / 1000 - fleece.status.updateDate > 60 * 60 * 24) {
      console.log('read from remote');/* eslint no-console: 0*/
      this.setState({
        status: Object.assign({}, this.state['status'], { needUpdate: true })
      }, () => {
        this._loadProfile();
      ...
      console.log('read from localstorage');/* eslint no-console: 0*/
  ...
  _beginLoading(): void {
    console.log('2');/* eslint no-console: 0*/
    this.setState(Object.assign({}, this.state['status'], { isLoading: true }));
  }

  _finishLoading(): void {
    console.log('3');/* eslint no-console: 0*/
    this.setState({
  ...
  render(): any {
    ...
    return (
      <div className="fleece-component">
        ...
        <div className="pinned-repos-component">
          <h5 className="content-container">{pinned_repos.length > 0 ? 'Pinned repositories' : ''}</h5>
